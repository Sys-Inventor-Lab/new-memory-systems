...
//EXPORT_SYMBOL(tp_page);
EXPORT_SYMBOL(swapped_list);
EXPORT_SYMBOL(vrt_swapped_table);
EXPORT_SYMBOL(phy_swapped_table);
...
int __isolate_lru_page(struct page *page, isolate_mode_t mode)
{
	...
	int reserve_granted;
	const unsigned int threshold = 0x30000000;

	reserve_granted = 0;

	hash_for_each_possible_safe(phy_swapped_table, ppage, tmp, node,
				     (u64)page) {
		if(ppage->page == page) {
			vpage = container_of(ppage->vrt_node_addr,
					     struct vrt_page, node);
			if(vpage->reuse_time >= threshold) {
				hash_del(&(ppage->node));
				hash_del(ppage->vrt_node_addr);
				kfree(ppage);
				kfree(vpage);
				reserve_granted = 1;
				break;
